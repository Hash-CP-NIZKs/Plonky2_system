...
mod tests {
    ...
    fn test_ecdsa() -> Result<()> {
        ...
        // for one exponentiation, internal snark need to prove 46 elliptice curve point addtions, 
        // 32 non-native multiplications(single execution have 2^{-4} soundness error, repetition for 32times) 
        // and a poseidon hash of 64 elements

        // in ecdsa, we prove (G^x)*(P^y), these two elliptic curve exponentiation
        // these two operations were main bottlenecks in previous works

        // the proof size is 147112 bytes + 64 field elements + 2 poseidon hash output
        ...
        const NUM_INSTANCES: usize = 64;//(parallet times)
    ...
    fn test_PoS() -> Result<()> {
        ...
        // for one exponentiation, internal snark need to prove 26 elliptice curve point addtions, 
        // 20 non-native multiplications(single execution have 2^{-3} soundness error, repetition for 20 times) 
        // and a poseidon hash of 42 elements


        // the proof size is 139336 bytes + 20 field elements + 1 poseidon hash output
        ...
        fn generate_random_3bit_u64() -> u64 {
            let mut rng = rand::thread_rng();
            let mut result = rng.gen_range(0..8) as u64;
        ...
        const NUM_INSTANCES: usize = 20;//(parallet times)
        const LIMB_SIZE: u64 = 1 << 32;
        ...
        let k_real: Vec<Vec<u32>> = (0..NUM_INSTANCES)
            ...
            .collect();
        let e_real: Vec<u64> = (0..NUM_INSTANCES)
            .map(|_| generate_random_3bit_u64())
            .collect();
        let lambda_real: Vec<u32> = generate_random_256bit_u32();
